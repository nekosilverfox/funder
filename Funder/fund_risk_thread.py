...
class FundRiskThread(QThread):
    ...
    def run(self):
        try:
            self.progress_signal.emit(f"获取基金 {self._fund_code} 风险信息中...")
            data = ak.fund_individual_analysis_xq(symbol=self._fund_code)
            self.progress_signal.emit(f"获取基金 {self._fund_code} 风险信息完成")

        except Exception as e:
            data = None
            error_message = f"获取基金 {self._fund_code} 风险信息时发生错误: {str(e)}"
